...
Register_Serializer(EthernetTypeOrLengthField, EthernetTypeOrLengthSerializer);
...
void EthernetTypeOrLengthSerializer::serialize(MemoryOutputStream& stream, const Ptr<const Chunk>& chunk) const
{
    const auto& header = staticPtrCast<const EthernetTypeOrLengthField>(chunk);
    stream.writeUint16Be(header->getTypeOrLength());
}

const Ptr<Chunk> EthernetTypeOrLengthSerializer::deserialize(MemoryInputStream& stream) const
{
